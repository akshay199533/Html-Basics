...
const Categories = () => {
  ...
  const category = categoriesArray.map((item,pos) => {
    return <div className="categories_menu_list" key={pos}>{item}</div>;
  });
  return (
    <div>
      <div className="categories">
        ...
        <div className="filter-wrapper">
          <img src={filterLogoUrl} />
          <p>Filter by Category</p>
        </div>
        <div className="categories_menu">
          <div className="categories_menu_list active">All</div>
          {category}
        </div>
      </div>
    </div>
